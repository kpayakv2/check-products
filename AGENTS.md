...
Every doc in this repo is exactly one of four kinds — **Rule**, **Status**, **Wayfinding**, **Memory** — defined in [CONTEXT.md](CONTEXT.md). This file is Rule's single canonical home: a rule lives here, or in a file this document explicitly names as where its detail lives (e.g. `.agents/rules/rules-windows.md`, cited below). Anywhere else that states a rule is either stale or wrong — fix it to link back here instead of restating it.
