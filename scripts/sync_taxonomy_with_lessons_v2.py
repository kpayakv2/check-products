...
try:
    ...
    res = supabase.table('taxonomy_nodes').select('id, name_th, code').execute()
    ...
    for i, (main, sub) in enumerate(pairs):
        ...
        if main not in db_nodes:
            code = f"cat_L_{i:03d}"
            print(f"➕ Creating Main Category: {main} ({code})")
        ...
        if sub not in db_nodes:
            code = f"cat_L_{i:03d}_{new_nodes_count:03d}"
            print(f"  ➕ Creating Sub Category: {sub} ({code}) (Parent: {main})")
    ...
    if new_nodes_count > 0:
        print("\n🧠 Triggering embedding generation for new categories...")
        # Since I have the script, I'll just run it
