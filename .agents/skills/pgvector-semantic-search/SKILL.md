...
## ⚠️ Project-Specific Override (check-products)

> **อ่านก่อนส่วนอื่น:** กฎด้านล่างนี้ Override ค่า Default ของ Skill นี้สำหรับโปรเจกต์ `check-products`

| Setting | Skill Default | **โปรเจกต์นี้** |
|---------|--------------|-----------------|
| Dimension | `1536` (OpenAI) | **`384`** (MiniLM-L12-v2) |
| Column Type | `halfvec(1536)` | **`vector(384)`** |
| Index ops | `halfvec_cosine_ops` | **`vector_cosine_ops`** |
| Distance | `<=>` | `<=>` (เหมือนกัน) |
| Model | OpenAI text-embedding | **`paraphrase-multilingual-MiniLM-L12-v2`** |
| Embedding Provider | External API | **FastAPI local `http://127.0.0.1:8000/api/embed`** |

### ตัวอย่างที่ถูกต้องสำหรับโปรเจกต์นี้
```sql
-- ✅ สำหรับโปรเจกต์ check-products
CREATE INDEX ON taxonomy_nodes USING hnsw (embedding vector_cosine_ops);
...
```python
# ✅ Generate embedding ผ่าน FastAPI local
import httpx
