...
│   ├── src/
│   │   ├── api/                             # FastAPI Backend (Modular)
│   │   │   ├── api_server.py                # ← Entry Point (รันตัวนี้)
│   │   │   ├── models.py                    # Pydantic Schemas
│   │   │   ├── dependencies.py              # Global State, Model Init
...
| **`scripts/complete_deduplication_pipeline.py`** | ระบบครบวงจร + Human-in-the-Loop | `python scripts/complete_deduplication_pipeline.py --input products.csv --mode analyze` |
| **`src/api/api_server.py`** | FastAPI REST API Server (Modular) | `START_PHAYAK.bat` หรือ `.venv\Scripts\python src\api\api_server.py` → http://127.0.0.1:8000 |
...
| ไฟล์ | หน้าที่ | ใช้โดย |
|------|---------|--------|
| **`src/core/fresh_implementations.py`** | Core algorithms:<br>- TextProcessor (Thai text cleaning)<br>- EmbeddingModel (TF-IDF, Sentence Transformers)<br>- SimilarityCalculator | ทุกไฟล์หลัก |
| **`src/core/fresh_architecture.py`** | Architecture components:<br>- ProductMatcher<br>- ProductSimilarityPipeline<br>- Config | main.py, src/api/* |
...
| ไฟล์ | หน้าที่ | URL |
|------|---------|-----|
| **`web/human_review.html`** | Web UI สำหรับ human review | http://127.0.0.1:8000/web |
| **`web/product_deduplication.html`** | Web UI สำหรับ deduplication | http://127.0.0.1:8000/dedup |
...
| **หาสินค้าที่ไม่ซ้ำ** | `scripts/complete_deduplication_pipeline.py` | `python scripts/complete_deduplication_pipeline.py --input products.csv --mode extract` |
| **เริ่ม API Server** | `src/api/api_server.py` | `START_PHAYAK.bat` หรือ `.venv\Scripts\python src\api\api_server.py` |
| **Human Review** | `scripts/complete_deduplication_pipeline.py` | `python scripts/complete_deduplication_pipeline.py --mode review` |
| **เทรน ML Model** | `scripts/complete_deduplication_pipeline.py` | `python scripts/complete_deduplication_pipeline.py --mode train` |
| **ทำความสะอาด CSV** | `clean_csv_products.py` | `python clean_csv_products.py input.csv` |
| **แก้ไข Text Processing** | `src/core/fresh_implementations.py` | (ThaiTextProcessor class) |
| **แก้ไข Embedding Model** | `src/core/fresh_implementations.py` | (EmbeddingModel class) |
| **แก้ไข Similarity** | `src/core/fresh_implementations.py` | (SimilarityCalculator class) |
| **แก้ไข Embed Endpoint** | `src/api/routers/embed.py` | — |
| **แก้ไข Match Endpoint** | `src/api/routers/match.py` | — |
...
python scripts/complete_deduplication_pipeline.py --input input/new_product/products.xlsx --mode extract

# 4. เริ่ม API Server + Frontend พร้อมกัน (แนะนำ)
START_PHAYAK.bat

# หรือรัน Backend เดี่ยว
.venv\Scripts\python src\api\api_server.py
# เปิด http://127.0.0.1:8000/docs

# 5. Human Review
python scripts/complete_deduplication_pipeline.py --input input/new_product/products.xlsx --mode review --reviewer "Your Name"
```
...
**ไฟล์สำคัญที่สุด**:
- 🐍 `src/api/api_server.py` - FastAPI Entry Point (Modular v4.0)
- 🐍 `scripts/complete_deduplication_pipeline.py` - Deduplication system
- 🐍 `src/core/fresh_implementations.py` - Core algorithms
...
- 🚀 `START_PHAYAK.bat` - รันทั้งระบบในคลิกเดียว
