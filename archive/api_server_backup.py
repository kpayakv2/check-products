...
try:
    ...
    print("⚠️ Supabase not installed. Category classification will be unavailable.")
    Client = None
...
app_state = {
    ...
    "category_classifier": None,  # Lazy initialization for category classifier
...
def initialize_category_classifier():
    ...
    if app_state["category_classifier"] is None:
        ...
        try:
            ...
            print(f"❌ Failed to initialize Category Classifier: {e}")
            raise
...
class CategoryClassifier:
    ...
    def load_taxonomy(self):
        ...
        try:
            ...
            if self.taxonomy_flat:
                print(f"   ✅ Loaded {len(self.taxonomy_flat)} taxonomy nodes")
                self._generate_category_embeddings()
            else:
                print("   ⚠️ No taxonomy nodes found")
        except Exception as e:
                ...
                if keyword.lower() in product_lower:
                    ...
                    if cat_id not in matches or matches[cat_id]['confidence'] < confidence:
                        match_data = {
                            ...
                            'category_level': cat_info.get('level', 0) if cat_info else 999
                        }
                        if cat_info:
    ...
    def classify_hybrid(self, product_name: str, top_k: int = 3) -> List[Dict]:
        """Classify using hybrid approach (Keyword + Embedding)."""
    ...
    def classify(self, product_name: str, method: str = 'hybrid', top_k: int = 3) -> List[Dict]:
        """Main classification method."""
        if method == 'keyword':
            return self.classify_keyword_based(product_name, top_k)
        elif method == 'embedding':
            return self.classify_embedding_based(product_name, top_k)
        else:
            return self.classify_hybrid(product_name, top_k)
    ...
    try:
        ...
        suggestion_models = [
            CategorySuggestion(
                ...
                category_level=s.get('category_level', 0),
                confidence=s['confidence'],
                method=s.get('method', request.method),
        ...
        return CategoryResponse(
            ...
            top_suggestion=suggestion_models[0] if suggestion_models else None,
            processing_time=round(processing_time, 3)
        )
