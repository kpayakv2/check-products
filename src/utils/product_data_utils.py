...
def extract_product_names(df: pd.DataFrame, column_hint: Optional[str] = None) -> List[str]:
    ...
    # REMOVED: Fallback to first column based on UX feedback
    # The user must explicitly select the column if auto-detection fails.
    
    logger.error("No suitable columns found for product names")
    raise ValueError(f"ไม่พบคอลัมน์ที่ระบุชื่อสินค้า กรุณาเลือกจากคอลัมน์ที่มี: {', '.join(cols)}")
