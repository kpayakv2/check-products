...
from src.core.fresh_architecture import (
...
class BasicTextProcessor(TextProcessor):
    ...
    def __init__(self, 
        ...
        # Modified to protect Thai character range \u0e00-\u0e7f
    ...
    def process(self, text: str) -> str:
        ...
        # Normalize unicode (Using NFC for Thai to preserve characters like SARA AM)
...
class ThaiTextProcessor(BasicTextProcessor):
    ...
    def process(self, text: str) -> str:
        ...
        # 1. Unicode Normalization (DISABLED: NFKC breaks Thai SARA AM)
        ...
        # 6. Protect decimal numbers and unit symbols before punctuation removal
        result = re.sub(r'(\d)\.(\d)', r'\1_DOT_\2', result)
        result = result.replace('"', '_INCH_')
        ...
        # 8. Restore decimal dots and unit symbols
        result = result.replace('_DOT_', '.')
        result = result.replace('_INCH_', '"')
        result = result.replace('_PERCENT_', '%')
            
        # 9. Whitespace Normalization or Whitespace Elimination (For Deduplication)
        if self.remove_all_spaces:
            # Completely strip all whitespace characters (including unicode spaces)
            result = re.sub(r'\s+', '', result)
            result = re.sub(r'[\u00A0\u1680\u2000-\u200B\u202F\u205F\u3000]', '', result)
        elif self.normalize_whitespace or self.standardize_spaces:
...
class ComponentFactory:
    ...
    def create_embedding_model(model_type: str = "mock", **kwargs) -> EmbeddingModel:
        ...
        if model_type == "mock":
            ...
            from src.core.advanced_models import OptimizedTFIDFModel
