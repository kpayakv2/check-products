...
from src.core.fresh_architecture import EmbeddingModel
...
class SentenceTransformerModel(EmbeddingModel):
    ...
    def __init__(self, 
        ...
        if cache_dir is None:
            ...
            if local_cache.exists():
                ...
                print(f"[Cache] Using local model cache: {local_cache}")
        
        # Check if model exists in cache for offline usage
        self._check_offline_model_availability(model_name, cache_dir)
        
        # Initialize model
        print(f"[Load] Loading Sentence Transformer model: {model_name}")
        try:
            if cache_dir:
            ...
            if "local_files_only" in str(e):
                print(f"[Offline] Model not available offline. Run download_models.py first.")
            raise
        
        print(f"[OK] Model loaded successfully! Dimension: {self.get_dimension()}")
    ...
    def _check_offline_model_availability(self, model_name: str, cache_dir: Optional[str]) -> bool:
        ...
        for pattern in model_cache_patterns:
            if (cache_path / pattern).exists():
                print(f"[OK] Model found in offline cache: {pattern}")
                return True
                
        print(f"[Warning] Model not found in offline cache: {model_name}")
        return False
    ...
    def enable_offline_mode(self, force_offline: bool = True):
        ...
        print(f"[Offline] Offline mode: {'enabled' if force_offline else 'disabled'}")
    ...
    def encode(self, texts: List[str]) -> np.ndarray:
        ...
        embeddings = self.model.encode(
            ...
            show_progress_bar=False     # Disable to prevent UnicodeEncodeError on Windows Console
        )
...
def check_offline_models(cache_dir: Optional[str] = None) -> Dict[str, bool]:
    ...
    """
    if cache_dir is None:
        cache_dir = str(Path(__file__).parent.parent.parent / "model_cache")
