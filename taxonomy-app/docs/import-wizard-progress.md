...
- [ ] **บั๊กใหม่ที่เจอ 28 ส.ค.:** `/api/v1/match/import-dedup` ดึงสินค้าฝั่งสต๊อกไม่มี `ORDER BY` และโดน PostgREST cap ที่ 1,000 แถว ทั้งที่สต๊อกมี 3,103 แถว — ตรวจของซ้ำจึงเทียบกับสต๊อกแค่ ~32% แบบสุ่มไม่คงที่ในแต่ละครั้ง ต้องเพิ่ม ordering + pagination
