...
import { Type, RefreshCcw, Home } from 'lucide-react'
...
export default function Error({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-10 left-10 w-[300px] h-[300px] bg-blue-50/30 rounded-full blur-[100px] pointer-events-none" />
      ...
      >
        <div className="w-24 h-24 bg-rose-50 rounded-full flex items-center justify-center mx-auto mb-8 border border-rose-100 shadow-inner">
           <Type className="w-10 h-10 text-rose-500" />
        </div>

        <h2 className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight">
          Synonyms Management Error
        </h2>
        <h3 className="text-xl font-bold text-slate-700 mb-4 thai-text">
          ข้อผิดพลาดในการจัดการคำพ้องความหมาย
        </h3>
        
        <p className="text-slate-500 mb-10 thai-text leading-relaxed">
          เกิดข้อผิดพลาดในการจัดการฐานข้อมูลคำพ้อง (Synonyms) กรุณาลองใหม่อีกครั้ง หรือติดต่อผู้ดูแลระบบครับ
        </p>

        <div className="flex flex-col gap-4">
          <button
            onClick={() => reset()}
            className="w-full flex items-center justify-center gap-3 bg-rose-600 text-white py-5 rounded-[24px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-100 group"
          >
        ...
        <div className="mt-10 pt-10 border-t border-slate-100/50">
           <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.3em]">
             Error ID: {error.digest || 'SYNONYM_ERR_UNKNOWN'}
           </p>
        </div>
      </motion.div>
    </div>
