...
import { Upload, RefreshCcw, Home } from 'lucide-react'
...
export default function Error({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-sky-50/50 rounded-full blur-[100px] -mr-48 -mt-48 pointer-events-none" />
      ...
      >
        <div className="w-24 h-24 bg-sky-50 rounded-full flex items-center justify-center mx-auto mb-8 border border-sky-100 shadow-inner">
        ...
        <h3 className="text-xl font-bold text-slate-700 mb-4 thai-text">
          ข้อผิดพลาดในการนำเข้าข้อมูล
        </h3>
        
        <p className="text-slate-500 mb-10 thai-text leading-relaxed">
          เกิดข้อผิดพลาดในการอัปโหลดหรือประมวลผลไฟล์ข้อมูลสินค้า กรุณาตรวจสอบความถูกต้องของไฟล์แล้วลองอีกครั้งครับ
        </p>

        <div className="flex flex-col gap-4">
          <button
            onClick={() => reset()}
            className="w-full flex items-center justify-center gap-3 bg-sky-600 text-white py-5 rounded-[24px] font-black uppercase tracking-widest hover:bg-sky-700 transition-all shadow-xl shadow-sky-100 group"
          >
        ...
        <div className="mt-10 pt-10 border-t border-slate-100/50">
           <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.3em]">
             Error ID: {error.digest || 'IMPORT_ERR_UNKNOWN'}
           </p>
        </div>
      </motion.div>
    </div>
