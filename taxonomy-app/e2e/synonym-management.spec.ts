...
/** POST/PUT/DELETE ไปที่ /api/synonyms ต้องมี session cookie (middleware.ts กันไว้) */
async function unlockOrSkip(context: BrowserContext) {
...
/**
 * แผงคำพ้องความหมาย (SynonymsPanel) ไม่มีหน้าของตัวเองแล้ว —
 * ยุบเข้าเป็นแท็บหนึ่งใน /taxonomy ตอนรวมหน้า taxonomy กับ synonyms เข้าด้วยกัน
 */
test.describe('Synonym Management', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.locator('[data-testid="tab-synonyms"]').click()
  ...
  test('should create new synonym', async ({ page, context }) => {
    await unlockOrSkip(context)
    await page.reload()
    await page.locator('[data-testid="tab-synonyms"]').click()
    await expect(page.locator('[data-testid="page-title"]')).toBeVisible()
  ...
  test('should edit existing synonym', async ({ page, context }) => {
    await unlockOrSkip(context)
    await page.reload()
    await page.locator('[data-testid="tab-synonyms"]').click()
    await expect(page.locator('[data-testid="page-title"]')).toBeVisible()
  ...
  test('should delete synonym', async ({ page, context }) => {
    await unlockOrSkip(context)
    await page.reload()
    await page.locator('[data-testid="tab-synonyms"]').click()
    await expect(page.locator('[data-testid="page-title"]')).toBeVisible()
