...
serve(async (req) => {
  ...
  try {
    ...
    const tokens = cleanText.split(/[\s\-\(\)\[\]\/\\,\.]+/).filter((t: string) => t.length >= 2)
    ...
    categories.forEach((category: any) => {
      ...
      category.keywords.forEach((keyword: string) => {
        ...
        // Token match
        else if (tokens.some((token: string) => token === keywordLower)) {
          score += 0.8
          matchedKeywords.push(keyword)
        }
        // Partial match
        else if (keywordLower.length >= 3 && tokens.some((token: string) => token.includes(keywordLower) || keywordLower.includes(token))) {
          score += 0.3
          matchedKeywords.push(keyword)
        }
      })
